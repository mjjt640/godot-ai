...
var _crypto: Crypto
var _last_heartbeat_timeout_warning_msec: int = -60000
var _heartbeat_timeout_count: int = 0
...
func set_panel(panel: Control) -> void:
	_panel = panel
	if _panel and _panel.has_method("set_server"):
		_panel.set_server(self)
	_update_panel_state()
...
func _on_heartbeat_timeout() -> void:
	_heartbeat_timeout_count += 1
	var now_msec := Time.get_ticks_msec()
	if now_msec - _last_heartbeat_timeout_warning_msec >= 60000:
		_last_heartbeat_timeout_warning_msec = now_msec
		push_warning("[MCP] Heartbeat timeout (suppressed repeated timeouts: %d)" % max(_heartbeat_timeout_count - 1, 0))
		_heartbeat_timeout_count = 0
...
func _update_panel_state() -> void:
	var panel = _get_panel()
	if panel and panel.has_method("update_server_state"):
		panel.update_server_state(get_server_state())

func get_server_state() -> Dictionary:
	return {
		"running": _current_port > 0,
		"host": "127.0.0.1",
		"port": _current_port,
		"client_count": _peers.size(),
		"authenticated_count": _authenticated_peers.size(),
