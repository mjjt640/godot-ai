...
将 `godot-ops.ts`（1112 行、15 个工具）完整拆分为 4 个新模块 + 1 个已有模块：
...
| `godot-ops.ts` | **删除**——15 个工具全部迁出，原文件归零 | 0 |

注意：execute_gdscript 在 `script.ts`，search_classes/get_class_info/get_inheritance/find_method 在 `docs.ts`，edit_script/project_replace 在 `script.ts`——这些工具不在 godot-ops.ts 中，无需迁移。
...
| 大文件拆分 | godot-ops.ts 1112 行删除（P1 完成，15 工具迁入 5 个模块） |
