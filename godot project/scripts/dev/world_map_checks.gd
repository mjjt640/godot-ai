...
func check_arena_visual_scene(ctx, collision_layers) -> void:
	...
	ctx.expect(obstacle_count > 0, "ArenaVisual should create resource-driven obstacles")
	ctx.remove_instance(instance)
...
func check_arena_hazards(ctx, collision_layers) -> void:
	...
	ctx.expect(not hazards.is_empty(), "ArenaVisual should create resource-driven hazards")
...
func check_default_map_data(ctx) -> void:
	...
	if run_tuning != null:
		ctx.expect(run_tuning.arena_half_extents.x >= 2400.0, "First playable map should be wide enough for survivor-style roaming")
		ctx.expect(run_tuning.arena_half_extents.y >= 1500.0, "First playable map should be tall enough for survivor-style roaming")
	...
	if visual_data != null:
		...
		ctx.expect(obstacles.size() >= 8, "First playable map should reuse simple obstacle chunks across a large arena")
		ctx.expect(obstacles.size() <= 16, "First playable map should stay simple and avoid maze-like obstacle density")
		ctx.expect(hazards.size() <= 2, "First playable map should keep hazards sparse while testing roaming")
...
func check_default_map_tile_library(ctx) -> void:
	...
	ctx.expect(tile_library.get("tile_size") == Vector2i(256, 256), "Cyber test zone tiles should keep 256x256 source cells")
	ctx.expect((tile_library.get("ground_base_tiles") as Array).size() >= 6, "Cyber test zone should have enough base ground tiles")
	ctx.expect((tile_library.get("ground_detail_tiles") as Array).size() >= 6, "Cyber test zone should have enough detail ground tiles")
	ctx.expect((tile_library.get("neon_detail_tiles") as Array).size() >= 4, "Cyber test zone should have neon detail tiles")
	ctx.expect((tile_library.get("decals") as Array).size() >= 16, "Cyber test zone should have transparent decal assets")
	ctx.expect((tile_library.get("hazard_visuals") as Array).size() >= 16, "Cyber test zone should have hazard visual assets")
	ctx.expect((tile_library.get("hazard_ground_tiles") as Array).size() >= 16, "Cyber test zone should have hazard ground tile assets")
